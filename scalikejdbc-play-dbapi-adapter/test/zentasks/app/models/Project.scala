...
object Project extends SQLSyntaxSupport[Project] {
  ...
  def findById(id: Long)(implicit s: DBSession = auto): Option[Project] =
    withSQL {
      select.from(Project as p).where.eq(p.id, id)
    }.map(Project(p)).single.apply()

  def findInvolving(user: String)(implicit s: DBSession = auto): Seq[Project] =
    withSQL {
      select
        .from(Project as p)
  ...
  def rename(id: Long, newName: String)(implicit s: DBSession = auto): Unit =
    applyUpdate {
  ...
  def deleteInFolder(folder: String)(implicit s: DBSession = auto): Unit =
    applyUpdate {
      deleteFrom(Project as p).where.eq(p.folder, folder)
    }

  def renameFolder(folder: String, newName: String)(implicit
    s: DBSession = auto
  ): Unit = applyUpdate {
    update(Project as p).set(p.folder -> newName).where.eq(p.folder, folder)
  }

  def membersOf(project: Long)(implicit s: DBSession = auto): Seq[User] =
    withSQL {
      select
        .from(User as u)
        .join(ProjectMember as m)
        .on(m.userEmail, u.email)
        .where
        .eq(m.projectId, project)
    }.map(User(u)).list.apply()

  def addMember(project: Long, user: String)(implicit
    s: DBSession = auto
  ): Unit = applyUpdate {
    insert.into(ProjectMember).values(project, user)
  }

  def removeMember(project: Long, user: String)(implicit
    s: DBSession = auto
  ): Unit = applyUpdate {
    deleteFrom(ProjectMember as m).where
      .eq(m.projectId, project)
      .and
      .eq(m.userEmail, user)
  }

  def isMember(project: Long, user: String)(implicit
    s: DBSession = auto
  ): Boolean = withSQL {
    select(sqls"count(${u.email}) = 1 as is_member")
      ...
      .and
      .eq(u.email, user)
  }.map(rs => rs.boolean("is_member").asInstanceOf[Boolean])
    .single
    .apply()
    .getOrElse(false)

  def create(project: NewProject, members: Seq[String])(implicit
    s: DBSession = auto
  ): Project = {
    // Insert the project
    val newId = sql"select next value for project_seq as v from dual"
      .map(_.long("v"))
      .single
      .apply()
      .get
    applyUpdate {
      insert.into(Project).values(newId, project.name, project.folder)
    }
    // Add members
    members foreach { email =>
      applyUpdate(insert.into(ProjectMember).values(newId, email))
    }
